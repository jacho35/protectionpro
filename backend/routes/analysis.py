...
@router.post("/open-conductor", response_model=OpenConductorResults)
def open_conductor(data: ProjectData):
    """Run one-conductor-open (single-phasing) series fault analysis on the
    cable branch identified by data.openConductorBranchId."""
    if not data.openConductorBranchId:
        raise HTTPException(status_code=400, detail="openConductorBranchId is required.")
    try:
        return run_open_conductor_analysis(data, data.openConductorBranchId,
                                           voltage_factor=data.voltageFactor)
