...
const FIELD_INFO = {
  ...
  // Cable
  'cable.temperature_c': 'Operating conductor temperature (°C) for an overhead line. The conductor library stores resistance at 20 °C (the BS 215 / IEC 61089 codeword convention), while the underground cable library stores HOT values (90 °C XLPE / 70 °C PVC) — so without this correction an overhead line would run cold in every engine and its losses and voltage drop would come out ~15–25 % optimistic.\nThe engines apply R(T) = R₂₀·[1 + α(T − 20)] with the conductor\'s own temperature coefficient (ACSR/AAC α ≈ 0.00403, AAAC ≈ 0.0036, copper ≈ 0.00393).\nDefault 75 °C — the conductor temperature the library\'s in-air `rated_amps` is quoted at, so resistance and ampacity describe the same conductor state. Lower it to model a lightly-loaded line, raise it for summer or emergency ratings.\nUsed by: Load Flow, Fault Analysis, Harmonics, Unbalanced Load Flow, Contingency.',
  'cable.z0_coupling': 'How the zero-sequence impedance of PARALLEL overhead circuits is modelled (num_parallel > 1).\nZero-sequence current is in phase in all three conductors and returns through earth, so two circuits on a shared tower are strongly coupled through that common earth return. The mutual term RAISES the effective Z₀ well above the naive Z₀/n — by ~1.7× on a typical MV double-circuit tower — which directly changes every earth-fault current on the line.\n• Auto — Carson mutual impedance from the circuit spacing and soil resistivity: Z₀_eff = [Z₀s + (n−1)·Z₀m]/n.\n• None — the old plain Z₀/n divide; correct only for genuinely independent circuits on separate routes.\n• Manual ratio — enter Z₀m/Z₀s directly from a line-parameter study.\nSingle circuits are unaffected. Used by: Fault Analysis (SLG / LLG), Unbalanced Load Flow (zero-sequence network).',
  'cable.circuit_spacing_m': 'Geometric mean distance between the two parallel circuits\' conductors (m) — the geometric mean of the nine inter-circuit conductor distances on a double-circuit tower. Smaller spacing means tighter zero-sequence coupling and a higher effective Z₀. Typical shared-tower value 6–12 m.\nUsed by: Fault Analysis, Unbalanced Load Flow (parallel Z₀ coupling, Auto mode).',
  'cable.soil_resistivity_ohm_m': 'Soil resistivity ρ (Ω·m) for Carson\'s earth-return depth D_e = 658.87·√(ρ/f). Higher resistivity pushes the return current deeper, increasing the mutual reactance between parallel circuits. Typical: 30 Ω·m wet clay, 100 Ω·m average soil, 1000 Ω·m dry sand/rock.\nUsed by: Fault Analysis, Unbalanced Load Flow (parallel Z₀ coupling, Auto mode).',
  'cable.z0_mutual_factor': 'Zero-sequence mutual/self impedance ratio Z₀m/Z₀s, entered directly (Manual mode). Typical shared-tower double circuits: 0.5–0.7. Capped at 0.95 — the mutual can never reach the self impedance.\nUsed by: Fault Analysis, Unbalanced Load Flow (parallel Z₀ coupling, Manual mode).',
  ...
  'relay.z3_margin_pct': 'Zone 3\'s computed reach (line + Z3 Next-Line % of the longest downstream line) is multiplied by this %, used by "Grade Distance Zones". 120% is typical — Zone 3 needs more margin than Zone 2 since its estimate accumulates error over a longer reach.',
  ...
  'generator.gov_model': 'Turbine-dynamics shape (independent of the Governor mode above, which controls droop vs isochronous reset).\n• First order — the single Governor Time Const lag above (default, byte-identical to legacy studies).\n• DEGOV1 — diesel: actuator lag then a transport dead-time (delayed onset), the electronic-diesel-governor signature.\n• GAST — gas turbine: fuel-valve lag then a turbine lag, fuel capped by a simplified limit.\n• TGOV1 — steam: governor lag then a reheat lead-lag.\n• HYGOV — hydro: a temporary-droop dashpot feeding a gate/water-column model — shows the classic "power dips before it rises" water-hammer response.\nStandard-SHAPED reduced-order models for qualitative OEM step-response character, not certified vendor parameter sets.\nUsed by: Transient Stability.',
  'generator.degov_td_s': 'DEGOV1 transport dead-time TD — the combustion/actuation delay before a fuel-valve change reaches the crankshaft. Typical 0.05–0.15 s (default 0.1 s); much beyond that starts to erode the isochronous loop\'s phase margin (a real delay-vs-integral-control trade-off).\nUsed by: Transient Stability (DEGOV1).',
  'generator.gast_t1_s': 'GAST fuel-valve actuator lag T1. Typical 0.2–0.6 s (default 0.4 s).\nUsed by: Transient Stability (GAST).',
  'generator.gast_t2_s': 'GAST turbine (combustion-to-power) lag T2. Typical 0.05–0.3 s (default 0.1 s).\nUsed by: Transient Stability (GAST).',
  'generator.gast_fuel_limit_pu': 'GAST fuel/temperature limit — a simplified constant cap on fuel command standing in for the full temperature-control loop. Default 1.15 p.u.\nUsed by: Transient Stability (GAST).',
  'generator.tgov1_t1_s': 'TGOV1 governor (steam-valve) lag T1. Typical 0.2–0.8 s (default 0.5 s).\nUsed by: Transient Stability (TGOV1).',
  'generator.tgov1_t2_s': 'TGOV1 reheat lead time T2. Typical 1–4 s (default 2.5 s).\nUsed by: Transient Stability (TGOV1).',
  'generator.tgov1_t3_s': 'TGOV1 reheat lag time T3. Typical 5–10 s (default 7 s).\nUsed by: Transient Stability (TGOV1).',
  'generator.hygov_tw_s': 'HYGOV water starting time Tw — the penstock inertia that produces the non-minimum-phase "power dips before it rises" hydro response. Typical 0.5–2 s (default 1 s).\nUsed by: Transient Stability (HYGOV).',
  'generator.hygov_gate_tg_s': 'HYGOV gate-servo time constant — deliberately its own (slower) constant, not the Governor Time Const above: a real hydro gate servo moves a large mechanical gate and is inherently much slower than an electronic diesel/gas actuator. Needs to stay well above Water Starting Time Tw for the loop to stay damped (default 1.5 s vs. Tw = 1 s) — too fast a gate chases the water hammer\'s initial wrong-direction dip and can ring.\nUsed by: Transient Stability (HYGOV).',
  'generator.hygov_rt_pct': 'HYGOV temporary droop Rt (%) — a dashpot feedback around the gate servo that damps the water-hammer interaction (a non-minimum-phase plant under integral/isochronous control is a textbook hard-to-damp combination; this is the classical remedy). Washes out at steady state, so it does not add a steady-state droop offset. Default 40%; isochronous HYGOV on a small/fast island is a genuinely sensitive tuning problem even so — droop mode and/or site-specific tuning is the practical answer, mirroring real hydro-governor practice.\nUsed by: Transient Stability (HYGOV).',
  'generator.hygov_dashpot_tr_s': 'HYGOV dashpot reset (washout) time. Default 6 s.\nUsed by: Transient Stability (HYGOV).',
  'generator.exc_model': 'Exciter error-path shape feeding the field (independent of the AVR Gain/Time Const above, which stay the outer gain/lag for First order and SEXS).\n• First order — the AVR Gain/Time Const lag above (default, byte-identical to legacy studies).\n• SEXS — measured terminal voltage filtered by a lead-lag before the same Ka/Ta lag.\n• ST1 — fast potential-source static exciter: Vt measurement lag, error lead-lag, then its own (typically faster) output lag.\n• AC — adds self-excitation and a core-saturation term that softens the ceiling (vs. the hard field-ceiling clip alone), plus a rate-feedback stabilising loop.\nUsed by: Transient Stability.',
  'generator.sexs_tb_s': 'SEXS terminal-voltage filter lag Tb. Typical 5–15 s (default 10 s).\nUsed by: Transient Stability (SEXS).',
  'generator.sexs_tc_s': 'SEXS terminal-voltage filter lead Tc. Typical 0.5–2 s (default 1 s).\nUsed by: Transient Stability (SEXS).',
  'generator.st1_tr_s': 'ST1 terminal-voltage measurement lag Tr. Typical 0.01–0.05 s (default 0.02 s).\nUsed by: Transient Stability (ST1).',
  'generator.st1_tb_s': 'ST1 error-path lag Tb (lead-lag pair with Tc). Default 1 s (≈ pass-through) unless tuned.\nUsed by: Transient Stability (ST1).',
  'generator.st1_tc_s': 'ST1 error-path lead Tc (lead-lag pair with Tb). Default 1 s (≈ pass-through) unless tuned.\nUsed by: Transient Stability (ST1).',
  'generator.st1_ta_s': 'ST1 output lag Ta — ST1\'s own (typically much faster) final lag, distinct from the outer AVR Time Const. Typical 0.01–0.05 s (default 0.02 s).\nUsed by: Transient Stability (ST1).',
  'generator.ac_tr_s': 'AC terminal-voltage measurement lag Tr. Typical 0.01–0.05 s (default 0.02 s).\nUsed by: Transient Stability (AC).',
  'generator.ac_ta_s': 'AC output lag Ta — the exciter field-voltage build-up time. Typical 0.1–0.5 s (default 0.2 s).\nUsed by: Transient Stability (AC).',
  'generator.ac_ke': 'AC self-excitation constant Ke. Default 1.0.\nUsed by: Transient Stability (AC).',
  'generator.ac_sat_a': 'AC core-saturation coefficient A in SE(Efd)=A·exp(B·|Efd|) — softens the field ceiling instead of relying on the hard clip alone. Default 0.05 (≈0 near rated field, growing past ~1.5×).\nUsed by: Transient Stability (AC).',
  'generator.ac_sat_b': 'AC core-saturation coefficient B in SE(Efd)=A·exp(B·|Efd|). Default 0.8.\nUsed by: Transient Stability (AC).',
  'generator.ac_kf': 'AC rate-feedback gain Kf — a minor stabilising loop on the field voltage. Default 0.03.\nUsed by: Transient Stability (AC).',
  'generator.ac_tf': 'AC rate-feedback washout time Tf. Default 1 s.\nUsed by: Transient Stability (AC).',
  'generator.pss_on': 'Power System Stabiliser — a washout + two lead-lag stages on speed deviation (IEEE PSS1A shape), summed into the active exciter\'s voltage-error input. The engine\'s only oscillation-damping capability: a lightly-damped swing rings noticeably less with the PSS on. Requires the AVR to be on.\nUsed by: Transient Stability.',
  'generator.pss_kstab': 'PSS gain Kstab. Typical 5–20 (default 10) — too high can destabilise rather than damp.\nUsed by: Transient Stability (PSS).',
  'generator.pss_tw_s': 'PSS washout time Tw — blocks the PSS from responding to steady-state speed offsets (e.g. droop), passing only the swing. Typical 5–15 s (default 10 s).\nUsed by: Transient Stability (PSS).',
  'generator.pss_t1_s': 'PSS first lead-lag stage lead time T1. Default 0.2 s — a mild (2:1) lead; a much larger T1/T2 ratio compounded over both stages can overshoot into destabilising phase territory for a typical local electromechanical mode (~1 Hz), so tune cautiously.\nUsed by: Transient Stability (PSS).',
  'generator.pss_t2_s': 'PSS first lead-lag stage lag time T2. Default 0.1 s.\nUsed by: Transient Stability (PSS).',
  'generator.pss_t3_s': 'PSS second lead-lag stage lead time T3. Default 0.2 s.\nUsed by: Transient Stability (PSS).',
  'generator.pss_t4_s': 'PSS second lead-lag stage lag time T4. Default 0.1 s.\nUsed by: Transient Stability (PSS).',
  'generator.pss_vs_limit_pu': 'PSS output clamp Vs_limit (p.u.) — bounds how much the stabiliser can perturb the exciter reference. Default 0.1 p.u.\nUsed by: Transient Stability (PSS).',
  ...
  'generator.trip_rocof_hzs': 'ROCOF (df/dt, ANSI 81R) protection: the generator trips if its island\'s rate of change of frequency, measured over a ~0.1 s window, stays above this for the trip delay (0 = disabled). Typical anti-islanding settings 0.5–1.0 Hz/s.\nUsed by: Transient Stability.',
  'generator.trip_oos_deg': 'Out-of-step / loss-of-synchronism protection (ANSI 78): the generator trips if its rotor angle relative to its island\'s centre of inertia exceeds this for the trip delay (0 = disabled). A separate, opt-in PROTECTIVE trip — distinct from the engine\'s global first-swing stability verdict, which never removes anything from the network. Typical 90–120°.\nUsed by: Transient Stability.',
  'generator.trip_vhz_pu': 'Over-excitation / volts-per-hertz protection (ANSI 24): the generator trips if its terminal V/Hz ratio (per-unit voltage ÷ per-unit frequency) stays above this for the trip delay (0 = disabled). Guards against core saturation on load rejection / over-excitation. Typical continuous limit ≈1.05, short-time up to ≈1.18–1.25 p.u.\nUsed by: Transient Stability.',
  'generator.trip_delay_s': 'Definite-time delay a frequency/voltage/ROCOF/out-of-step/V-Hz violation must persist before the generator trips. Typical 0.1–0.5 s (default 0.2 s).\nUsed by: Transient Stability.',
  ...
  'ibr_rocof_hzs': 'ROCOF (df/dt) anti-islanding: trip the inverter if its island\'s rate of change of frequency, measured over a ~0.1 s window, stays above this (Hz/s) for longer than the ride-through delay. 0 disables. Typical anti-islanding settings 0.5–1.0 Hz/s.\nUsed by: Transient Stability.',
  'ibr_oos_deg': 'Out-of-step / loss-of-synchronism: trip the grid-forming converter (a virtual synchronous machine with its own rotor angle) if its angle relative to its island\'s centre of inertia stays above this (°) for longer than the ride-through delay. 0 disables. No angle state exists for a grid-following converter (a current source), so this only applies in grid-forming mode.\nUsed by: Transient Stability.',
  'ibr_trip_delay_s': 'Ride-through time: how long a voltage/frequency/ROCOF/out-of-step violation must persist before the inverter trips. Model the grid-code ride-through window here (e.g. 0.15–0.5 s).\nUsed by: Transient Stability.',
...
const DB_LOAD_TYPES = [
  { key: 'lighting',   label: 'Lighting',          va: 100,  unit: 'point',   df: 1.0, poles: '1P', breaker_a: 10, curve: 'B', cable_mm2: 1.5, ecc_mm2: 1.5, per_circuit: 10, leak_ma: 0.4, pf: 0.95 },
  { key: 'socket',     label: 'Socket Outlet',     va: 200,  unit: 'socket',  df: 0.4, poles: '1P', breaker_a: 20, curve: 'B', cable_mm2: 2.5, ecc_mm2: 2.5, per_circuit: 6,  leak_ma: 0.75, pf: 0.9 },
  { key: 'geyser',     label: 'Geyser',            va: 3000, unit: 'geyser',  df: 1.0, poles: '1P', breaker_a: 20, curve: 'C', cable_mm2: 2.5, ecc_mm2: 2.5, per_circuit: 1,  leak_ma: 2.25, pf: 1.0 },
  { key: 'stove',      label: 'Stove / Oven',      va: 6000, unit: 'stove',   df: 1.0, poles: '1P', breaker_a: 40, curve: 'C', cable_mm2: 6,   ecc_mm2: 6,   per_circuit: 1,  leak_ma: 4.5, pf: 1.0 },
  { key: 'aircon',     label: 'Air Conditioner',   va: 2500, unit: 'unit',    df: 1.0, poles: '1P', breaker_a: 20, curve: 'C', cable_mm2: 2.5, ecc_mm2: 2.5, per_circuit: 1,  leak_ma: 1.5, pf: 0.85 },
  { key: 'ev_charger', label: 'EV Charger',        va: 7400, unit: 'charger', df: 1.0, poles: '1P', breaker_a: 40, curve: 'C', cable_mm2: 6,   ecc_mm2: 6,   per_circuit: 1,  leak_ma: 3.5, pf: 0.98 },
  { key: 'heat_pump',  label: 'Heat Pump',         va: 1500, unit: 'unit',    df: 1.0, poles: '1P', breaker_a: 16, curve: 'C', cable_mm2: 2.5, ecc_mm2: 2.5, per_circuit: 1,  leak_ma: 1.5, pf: 0.85 },
  { key: 'motor_3ph',  label: 'Motor (3φ)',        va: 4000, unit: 'motor',   df: 0.8, poles: '3P', breaker_a: 16, curve: 'D', cable_mm2: 2.5, ecc_mm2: 2.5, per_circuit: 1,  leak_ma: 2.0, pf: 0.85 },
  { key: 'spare',      label: 'Spare',             va: 0,    unit: 'way',     df: 1.0, poles: '1P', breaker_a: 20, curve: 'C', cable_mm2: 2.5, ecc_mm2: 2.5, per_circuit: 1,  leak_ma: 0, pf: 1.0 },
  ...
  { key: 'feeder_db',  label: 'Feeder to Sub-board', va: 0,  unit: 'board',   df: 1.0, poles: '3P', breaker_a: 63, curve: 'C', cable_mm2: 25,  ecc_mm2: 16,  per_circuit: 1,  leak_ma: 0, pf: 0.9 },
];
...
const COMPONENT_DEFS = {
  ...
  generator: {
    ...
    fields: [
      ...
      { key: 'gov_model', label: 'Turbine Model', type: 'select', options: ['first_order', 'degov1', 'gast', 'tgov1', 'hygov'], section: 'stability' },
      { key: 'degov_td_s', label: 'DEGOV1 Dead Time', type: 'number', unit: 's', min: 0.02, step: 0.05, section: 'stability', showWhen: { field: 'gov_model', values: ['degov1'] } },
      { key: 'gast_t1_s', label: 'GAST Fuel Valve Lag', type: 'number', unit: 's', min: 0.05, step: 0.05, section: 'stability', showWhen: { field: 'gov_model', values: ['gast'] } },
      { key: 'gast_t2_s', label: 'GAST Turbine Lag', type: 'number', unit: 's', min: 0.02, step: 0.02, section: 'stability', showWhen: { field: 'gov_model', values: ['gast'] } },
      { key: 'gast_fuel_limit_pu', label: 'GAST Fuel Limit', type: 'number', unit: 'p.u.', min: 0.5, step: 0.05, section: 'stability', showWhen: { field: 'gov_model', values: ['gast'] } },
      { key: 'tgov1_t1_s', label: 'TGOV1 Governor Lag', type: 'number', unit: 's', min: 0.05, step: 0.05, section: 'stability', showWhen: { field: 'gov_model', values: ['tgov1'] } },
      { key: 'tgov1_t2_s', label: 'TGOV1 Reheat Lead', type: 'number', unit: 's', min: 0.1, step: 0.1, section: 'stability', showWhen: { field: 'gov_model', values: ['tgov1'] } },
      { key: 'tgov1_t3_s', label: 'TGOV1 Reheat Lag', type: 'number', unit: 's', min: 0.5, step: 0.5, section: 'stability', showWhen: { field: 'gov_model', values: ['tgov1'] } },
      { key: 'hygov_tw_s', label: 'HYGOV Water Starting Time', type: 'number', unit: 's', min: 0.1, step: 0.1, section: 'stability', showWhen: { field: 'gov_model', values: ['hygov'] } },
      { key: 'hygov_gate_tg_s', label: 'HYGOV Gate Servo Time', type: 'number', unit: 's', min: 0.5, step: 0.1, section: 'stability', showWhen: { field: 'gov_model', values: ['hygov'] } },
      { key: 'hygov_rt_pct', label: 'HYGOV Temporary Droop', type: 'number', unit: '%', min: 5, max: 200, step: 5, section: 'stability', showWhen: { field: 'gov_model', values: ['hygov'] } },
      { key: 'hygov_dashpot_tr_s', label: 'HYGOV Dashpot Reset Time', type: 'number', unit: 's', min: 0.5, step: 0.5, section: 'stability', showWhen: { field: 'gov_model', values: ['hygov'] } },
      ...
      { key: 'exc_model', label: 'Exciter Model', type: 'select', options: ['first_order', 'sexs', 'st1', 'ac'], section: 'stability', showWhen: { field: 'avr_mode', values: ['on'] } },
      { key: 'sexs_tb_s', label: 'SEXS Filter Lag Tb', type: 'number', unit: 's', min: 0.1, step: 0.5, section: 'stability', showWhen: { field: 'exc_model', values: ['sexs'] } },
      { key: 'sexs_tc_s', label: 'SEXS Filter Lead Tc', type: 'number', unit: 's', min: 0.05, step: 0.1, section: 'stability', showWhen: { field: 'exc_model', values: ['sexs'] } },
      { key: 'st1_tr_s', label: 'ST1 Vt Measurement Lag', type: 'number', unit: 's', min: 0.005, step: 0.005, section: 'stability', showWhen: { field: 'exc_model', values: ['st1'] } },
      { key: 'st1_tb_s', label: 'ST1 Error Lag Tb', type: 'number', unit: 's', min: 0.05, step: 0.05, section: 'stability', showWhen: { field: 'exc_model', values: ['st1'] } },
      { key: 'st1_tc_s', label: 'ST1 Error Lead Tc', type: 'number', unit: 's', min: 0.05, step: 0.05, section: 'stability', showWhen: { field: 'exc_model', values: ['st1'] } },
      { key: 'st1_ta_s', label: 'ST1 Output Lag Ta', type: 'number', unit: 's', min: 0.005, step: 0.005, section: 'stability', showWhen: { field: 'exc_model', values: ['st1'] } },
      { key: 'ac_tr_s', label: 'AC Vt Measurement Lag', type: 'number', unit: 's', min: 0.005, step: 0.005, section: 'stability', showWhen: { field: 'exc_model', values: ['ac'] } },
      { key: 'ac_ta_s', label: 'AC Output Lag Ta', type: 'number', unit: 's', min: 0.02, step: 0.02, section: 'stability', showWhen: { field: 'exc_model', values: ['ac'] } },
      { key: 'ac_ke', label: 'AC Self-excitation Ke', type: 'number', min: 0, step: 0.1, section: 'stability', showWhen: { field: 'exc_model', values: ['ac'] } },
      { key: 'ac_sat_a', label: 'AC Saturation A', type: 'number', min: 0, step: 0.01, section: 'stability', showWhen: { field: 'exc_model', values: ['ac'] } },
      { key: 'ac_sat_b', label: 'AC Saturation B', type: 'number', min: 0, step: 0.1, section: 'stability', showWhen: { field: 'exc_model', values: ['ac'] } },
      { key: 'ac_kf', label: 'AC Rate-feedback Kf', type: 'number', min: 0, step: 0.01, section: 'stability', showWhen: { field: 'exc_model', values: ['ac'] } },
      { key: 'ac_tf', label: 'AC Rate-feedback Tf', type: 'number', unit: 's', min: 0.1, step: 0.1, section: 'stability', showWhen: { field: 'exc_model', values: ['ac'] } },
      { key: 'pss_on', label: 'Power System Stabiliser', type: 'select', options: ['off', 'on'], section: 'stability', showWhen: { field: 'avr_mode', values: ['on'] } },
      { key: 'pss_kstab', label: 'PSS Gain Kstab', type: 'number', min: 0, step: 1, section: 'stability', showWhen: { field: 'pss_on', values: ['on'] } },
      { key: 'pss_tw_s', label: 'PSS Washout Tw', type: 'number', unit: 's', min: 1, step: 1, section: 'stability', showWhen: { field: 'pss_on', values: ['on'] } },
      { key: 'pss_t1_s', label: 'PSS Lead T1', type: 'number', unit: 's', min: 0.05, step: 0.05, section: 'stability', showWhen: { field: 'pss_on', values: ['on'] } },
      { key: 'pss_t2_s', label: 'PSS Lag T2', type: 'number', unit: 's', min: 0.02, step: 0.01, section: 'stability', showWhen: { field: 'pss_on', values: ['on'] } },
      { key: 'pss_t3_s', label: 'PSS Lead T3', type: 'number', unit: 's', min: 0.05, step: 0.05, section: 'stability', showWhen: { field: 'pss_on', values: ['on'] } },
      { key: 'pss_t4_s', label: 'PSS Lag T4', type: 'number', unit: 's', min: 0.02, step: 0.01, section: 'stability', showWhen: { field: 'pss_on', values: ['on'] } },
      { key: 'pss_vs_limit_pu', label: 'PSS Output Limit', type: 'number', unit: 'p.u.', min: 0.01, step: 0.01, section: 'stability', showWhen: { field: 'pss_on', values: ['on'] } },
      ...
      { key: 'trip_rocof_hzs', label: 'ROCOF Trip', type: 'number', unit: 'Hz/s', min: 0, step: 0.1, section: 'protection' },
      { key: 'trip_oos_deg', label: 'Out-of-step Trip', type: 'number', unit: '°', min: 0, max: 180, step: 5, section: 'protection' },
      { key: 'trip_vhz_pu', label: 'Over-excitation (V/Hz) Trip', type: 'number', unit: 'p.u.', min: 0, step: 0.02, section: 'protection' },
  ...
  solar_pv: {
    ...
    fields: [
      ...
      { key: 'ibr_of_hz', label: 'IBR Over-freq Trip', type: 'number', unit: 'Hz', min: 0, step: 0.1, section: 'protection',
        showWhen: { field: 'ibr_ctrl', values: ['grid_following', 'grid_forming'] } },
      { key: 'ibr_rocof_hzs', label: 'IBR ROCOF Trip', type: 'number', unit: 'Hz/s', min: 0, step: 0.1, section: 'protection',
        showWhen: { field: 'ibr_ctrl', values: ['grid_following', 'grid_forming'] } },
      { key: 'ibr_oos_deg', label: 'IBR Out-of-step Trip', type: 'number', unit: '°', min: 0, max: 180, step: 5, section: 'protection',
        showWhen: { field: 'ibr_ctrl', values: ['grid_forming'] } },
  ...
  bus: {
    ...
    defaults: {
      ...
      two_layer_soil: 'off',
      soil_resistivity_lower: 100,
      upper_layer_thickness: 3.0,
      grid_length: 30,
      grid_width: 30,
      grid_depth: 0.5,
      num_conductors_x: 6,
  ...
  cable: {
    ...
    fields: [
      ...
      { key: 'temperature_c', label: 'Conductor Temperature', type: 'number', unit: '°C', min: -20, max: 250, step: 1, section: 'fault', showWhen: { field: 'construction', values: ['overhead'] } },
      { key: 'z0_coupling', label: 'Parallel Z₀ Coupling', type: 'select', section: 'fault', options: [{ value: '', label: 'Auto (shared tower)' }, { value: 'none', label: 'None (independent circuits)' }, { value: 'manual', label: 'Manual ratio' }], showWhen: { field: 'construction', values: ['overhead'] } },
      { key: 'circuit_spacing_m', label: 'Circuit Spacing (GMD)', type: 'number', unit: 'm', min: 0.1, step: 0.5, section: 'fault', showWhen: { field: 'construction', values: ['overhead'] } },
      { key: 'soil_resistivity_ohm_m', label: 'Soil Resistivity', type: 'number', unit: 'Ω·m', min: 1, step: 10, section: 'fault', showWhen: { field: 'construction', values: ['overhead'] } },
      { key: 'z0_mutual_factor', label: 'Manual Z₀m / Z₀s Ratio', type: 'number', min: 0, max: 0.95, step: 0.05, section: 'fault', showWhen: { field: 'z0_coupling', values: ['manual'] } },
  ...
  relay: {
    ...
    fields: [
      ...
      { key: 'z1_margin_pct', label: 'Z1 Margin', type: 'number', unit: '%', min: 50, max: 100, step: 1, showWhen: { field: 'relay_type', values: ['21'] }, section: 'protection' },
  ...
  switch: {
    ...
    fields: [
      { key: 'name', label: 'Name', type: 'text' },
      { key: 'switch_type', label: 'Switch Type', type: 'select',
        options: [
  ...
  distribution_board: {
    ...
    defaults: {
      ...
      // which reproduce the undegraded behaviour the check had before.
      way_install: null,
      // Measured external earth-loop impedance Ze (Ω). When set it overrides
      // the value derived from the SLD, for a board whose supply is known from
      // a test certificate rather than modelled.
      ze_ohm: null,
    ...
    fields: [
      ...
      { key: 'ze_ohm', label: 'External Loop Ze', type: 'number', unit: 'Ω', min: 0, step: 0.01, section: 'cable_sizing' },
      { key: 'essential', label: 'Essential (Backup) Load', type: 'select', options: ['yes', 'no'], section: 'loadflow' },
      { key: 'motor_fraction', label: 'Motor Fraction (0 = none)', type: 'number', min: 0, max: 1, step: 0.05, section: 'fault' },
      { key: 'motor_lrc_ratio', label: 'Motor LRC Ratio', type: 'number', min: 1, max: 10, step: 0.5, section: 'fault' },
    ],
