...
const BOQ = {
  ...
  opts: null,        // { sources: {demand, plan, sld, db}, waste, merge, zero }
  ...
  available() {
    return { demand: this._hasDemand(), plan: this._hasPlan(), sld: this._hasSLD(), db: this._hasDB() };
  },
  defaultSources() {
    const av = this.available();
    const t = AppState.projectType;
    if (t === 'retic') return { demand: av.demand, plan: av.plan, sld: false, db: false };
    if (t === 'building') return { demand: false, plan: av.plan, sld: av.sld, db: av.db };
    return av;
  },
  ...
  collect(opts) {
    opts = opts || this.opts || {};
    const inc = Object.assign({ demand: true, plan: true, sld: true, db: true }, opts.sources || {});
    ...
    return { lines: [...lines.values()], warnings, notes };
  },

  // Take-off + rates → priced, sectioned bill.
  compute(opts) {
    opts = opts || this.opts || this._defaultOpts();
    const { lines, warnings, notes } = this.collect(opts);
    const order = Rates.CATS.map(c => c.id);
    const out = { sections: [], total: 0, nItems: 0, nPriced: 0, cableM: 0, missing: [], warnings, notes };
    for (const c of order) {
      ...
      out.sections.push({ cat: c, label: this.SECTION[c], lines: ls, subtotal });
      out.total += subtotal;
      out.nItems += ls.length;
      out.nPriced += ls.filter(l => l.rate != null).length;
      out.cableM += ls.filter(l => l.unit === 'm' && l.cat === 'cable').reduce((s, l) => s + l.qty, 0);
      out.missing.push(...ls.filter(l => l.rate == null));
    }
    return out;
  },
  ...
  render() {
    ...
    res.sections.forEach((s, i) => {
      body += `<tr class="bq-grp"><td colspan="7">${letters[i]} · ${escHtml(s.label)}${s.cat === 'cable' && o.waste !== false ? ' <span class="rt-note-i">(quantities include waste)</span>' : ''}</td></tr>`;
      for (const l of s.lines) {
        const nr = l.rate == null;
        body += `<tr${nr ? ' class="bq-miss"' : ''}><td class="rt-code">${escHtml(l.key)}</td><td title="${escHtml(l.desc)}">${escHtml(l.desc)}</td><td>${l.from.map(f => `<span class="bq-src">${escHtml(f)}</span>`).join('')}</td>
          <td class="rt-num" title="${l.waste ? `Measured ${Rates._group(l.measured.toFixed(l.unit === 'm' ? 1 : 0))} + ${l.waste}% waste` : ''}">${qtyTxt(l)}</td><td>${escHtml(l.unit)}</td>
          <td class="rt-num">${nr ? '<span class="bq-norate">No rate</span>' : money(l.rate)}</td><td class="rt-num">${nr ? '—' : money(l.amount)}</td></tr>`;
      }
      body += `<tr class="bq-sub"><td></td><td colspan="5">${escHtml(s.label)} subtotal</td><td class="rt-num">${money(s.subtotal)}</td></tr>`;
    });
    if (res.sections.length) body += `<tr class="bq-sub bq-total"><td></td><td colspan="5">Total excl. VAT</td><td class="rt-num">${money(res.total)}</td></tr>`;
    const anySrc = Object.keys(o.sources).some(k => o.sources[k] && av[k]);
    m.querySelector('#bq-main').innerHTML = `
      <div class="bq-sum">
        <div class="bq-stat"><span class="k">Total excl. VAT</span><span class="v">${money(res.total)}</span></div>
        <div class="bq-stat"><span class="k">Lines</span><span class="v">${res.nItems} <small>items</small></span></div>
        <div class="bq-stat"><span class="k">Cable, all types</span><span class="v">${Rates._group(Math.round(res.cableM).toString())} <small>m${o.waste !== false ? ' incl. waste' : ''}</small></span></div>
        <div class="bq-stat"><span class="k">Priced</span><span class="v">${res.nPriced} / ${res.nItems} <small>lines</small></span></div>
      </div>
      ${warnHtml}${issuesHtml}
      <div class="rt-tablewrap">${res.sections.length ? `<table class="rt-tbl rt-ro-tbl bq-tbl"><thead><tr><th class="rt-c-key">Item code</th><th>Description</th><th style="width:230px">From</th><th class="rt-num" style="width:86px">Qty</th><th style="width:42px">Unit</th><th class="rt-num" style="width:112px">Rate</th><th class="rt-num" style="width:128px">Amount</th></tr></thead><tbody>${body}</tbody></table>`
  ...
  _aoa(res) {
    ...
    const rows = [['Item code', 'Section', 'Description', 'From', 'Qty', 'Unit', `Rate (${cur})`, `Amount (${cur})`, 'Supplier code']];
    for (const s of res.sections) {
      for (const l of s.lines) rows.push([l.key, s.label, l.desc, l.from.join('; '), l.qty, l.unit, l.rate == null ? 'No rate' : l.rate, l.amount == null ? '' : l.amount, l.supplier || '']);
      rows.push(['', s.label + ' subtotal', '', '', '', '', '', +s.subtotal.toFixed(2), '']);
    }
    rows.push([], ['', 'Total excl. VAT', '', '', '', '', '', +res.total.toFixed(2), '']);
  ...
  exportFile(kind) {
    ...
    if (kind === 'csv') {
      ...
      ws['!cols'] = [{ wch: 22 }, { wch: 26 }, { wch: 44 }, { wch: 34 }, { wch: 10 }, { wch: 6 }, { wch: 12 }, { wch: 14 }, { wch: 16 }];
  ...
  async exportPDF() {
    ...
    doc.text(doc.splitTextToSize(`Quantities from: ${srcs || '—'}.  ${this.opts && this.opts.waste === false ? 'No waste allowance.' : 'Quantities include each item’s waste allowance.'}`, 182), margin, margin + 16);
    doc.setFont('helvetica', 'bold');
    doc.text(`Total excl. VAT: ${Rates.money(res.total)}    ${res.nPriced} of ${res.nItems} lines priced`, margin, margin + 26);
    doc.setFont('helvetica', 'normal');
    const body = [];
    res.sections.forEach((s, i) => {
      body.push([{ content: `${'ABCDEFG'[i]} · ${s.label}`, colSpan: 6, styles: { fontStyle: 'bold', fillColor: [232, 238, 246] } }]);
      for (const l of s.lines) body.push([l.key, l.desc, l.unit === 'm' ? l.qty.toFixed(1) : String(l.qty), l.unit, l.rate == null ? 'No rate' : Rates.num2(l.rate), l.amount == null ? '—' : Rates.num2(l.amount)]);
      body.push([{ content: `${s.label} subtotal`, colSpan: 5, styles: { fontStyle: 'bold', halign: 'right' } }, { content: Rates.num2(s.subtotal), styles: { fontStyle: 'bold' } }]);
    });
    body.push([{ content: 'Total excl. VAT', colSpan: 5, styles: { fontStyle: 'bold', halign: 'right' } }, { content: Rates.num2(res.total), styles: { fontStyle: 'bold' } }]);
    doc.autoTable({
      startY: margin + 31,
      margin: { left: margin, right: margin },
      head: [['Item code', 'Description', 'Qty', 'Unit', `Rate (${cur})`, `Amount (${cur})`]],
      body,
      styles: { fontSize: 7.5, cellPadding: 1.4 },
      headStyles: { fillColor: [0, 120, 215], textColor: 255, fontStyle: 'bold' },
      columnStyles: { 0: { cellWidth: 36 }, 2: { halign: 'right', cellWidth: 18 }, 3: { cellWidth: 10 }, 4: { halign: 'right', cellWidth: 24 }, 5: { halign: 'right', cellWidth: 28 } },
      didParseCell: (d) => { if (d.section === 'body' && d.column.index === 4 && d.cell.raw === 'No rate') d.cell.styles.textColor = [200, 100, 0]; },
    });
    ...
    if (notes.length) {
      ...
      for (const n of notes) {
        ...
        if (y + lines.length * 4 > 285) { doc.addPage(); y = margin; }
    ...
    for (let i = 1; i <= pages; i++) {
      ...
      doc.text(`ProtectionPro · Bill of quantities · page ${i} of ${pages}`, margin, 292);
      doc.setTextColor(0);
    }
