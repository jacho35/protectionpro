...
- A series chain of cables/transformers between two real buses with no intervening bus is collapsed into one branch. A chain with exactly ONE tapped transformer plus any number of cables (before/after, either order) is modelled EXACTLY via `_reduce_chain_two_port` — a Kron reduction of the chain's own internal nodes, not a naive impedance sum — so a cable's impedance is never mis-referred through the tap ratio. Two-or-more cascaded tapped transformers sharing a chain with a cable is a separate, still-open case (falls back to the lumped approximation, still warned) — draw a bus at each transformer terminal for exact results there.
